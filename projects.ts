...
(() => {
  ...
  function prettify(repoName: string): string {
    ...
    return repoName
      ...
      .filter(Boolean)
      .map((w) =>
        acronyms.includes(w.toLowerCase())
          ? w.toUpperCase()
  ...
  function toProject(repo: GitHubRepo): Project | null {
    if (repo.fork || repo.archived || EXCLUDE.includes(repo.name)) return null;
  ...
  async function loadRepos(): Promise<GitHubRepo[]> {
    try {
      ...
      if (cached) {
        const { time, data } = JSON.parse(cached) as { time: number; data: GitHubRepo[] };
        if (Date.now() - time < CACHE_MS) return data;
      }
    } catch {
  ...
  function renderProject(p: Project): HTMLLIElement {
    ...
    links.append(
      link(p.liveUrl, "Live demo", "live-link"),
      link(p.codeUrl, "Code", "code-link")
    );
    head.append(links);
  ...
  function showStatus(container: HTMLElement, message: string, withProfileLink = false): void {
  ...
  async function init(): Promise<void> {
    ...
    try {
      ...
      const projects = sortProjects(
        repos.map(toProject).filter((p): p is Project => p !== null)
      ).slice(0, MAX_PROJECTS);

      if (projects.length === 0) {
        showStatus(
          container,
          "Abhi koi live project nahi mila. Repo ke About mein Website link daalein, wo yahan aa jayega."
        );
        return;
      }
